...
public class MLQScheduler {
    ...
    public static void main(String[] args) {
        System.out.println("Starting MLQ Scheduler...");

        Properties properties = new Properties();
        try (InputStream input = MLQScheduler.class.getResourceAsStream("/config.properties")) {
            if (input == null) {
        ...
        try {
            ...
            MLQDatacenterBroker broker = createBroker("Broker_0");
            ...
            broker.scheduleTasksToVms(); // Make sure to call this to initialize scheduling
            ...
            System.out.println(MLQScheduler.class.getName() + " finished!");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static MLQDatacenterBroker createBroker(String name) throws Exception {
        return new MLQDatacenterBroker(name);
    }
    ...
    private static List<Cloudlet> readCloudletsFromFile(String filename, int userId) {
        ...
        try (InputStream inputStream = MLQScheduler.class.getResourceAsStream(filename);
